...
client.on('ready', () => {
   voiceChannel.join();
    setInterval(() => {
     connection.playFile('./axelon.wav'); 
    }, 40000); // Runs this every 10 seconds.
});
...
client.on('guildMemberAdd', async member => {
  const channel = member.guild.channels.find(ch => ch.name === '🌌recepcion🌌');
    ...
    ctx.fillText(`${member.displayName}`, canvas.width/5, canvas.height / 1.9);
    ...
    ctx.drawImage(avatar, 245, 60, 145, 145);
